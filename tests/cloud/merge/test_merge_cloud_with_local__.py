...
from mediaserver_api import DependentSystemBoundToCloud
...
def _test_merge_cloud_with_local(cloud_host, distrib_url, two_vm_types, api_version, exit_stack):
    ...
    with assert_raises(DependentSystemBoundToCloud):
        merge_systems(two, one, take_remote_settings=False)
    # Merge systems (takeRemoteSettings = true)
    merge_systems(two, one, take_remote_settings=True, timeout_sec=60)
    wait_for_settings_merge(one, two)
    settings_to_check = {
        k: v for k, v in two.api.get_system_settings().items() if k in TEST_SYSTEM_SETTINGS.keys()
        }
