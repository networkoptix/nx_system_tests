...
class test_export_low_resolution_video_with_overlays(VMSTest):
    """Export low resolution video with overlays.
    ...
    def _run(self, args, exit_stack):
        ...
        exit_stack.enter_context(playing_testcamera(machine_pool, server_vm.os_access, 'samples/overlay_test_video_lowres.mp4'))
        ...
        server_vm.default_archive().camera_archive(test_camera_1.physical_id).save_media_sample(
            ...
            SampleMediaFile(gui_prerequisite_store.fetch('samples/overlay_test_video_lowres.mp4')),
            )
        ...
        export_settings_1.export_with_specific_path(client_installation.temp_dir() / 'temp_overlay20880_2.mp4')
        ...
        export_settings_2.preview.get_text_position()
        export_settings_2.preview.get_image_position()
        export_settings_2.preview.get_timestamp_position()
        export_settings_2.preview.get_bookmark_position()
        loaded_preview_lowres = SavedImage(gui_prerequisite_store.fetch('test20880/preview_lowres.png'))
        actual_preview = export_settings_2.capture_preview()
        assert actual_preview.is_similar_to(loaded_preview_lowres)

        timestamp_position = export_settings_2.preview.get_timestamp_position()
        export_settings_2.export_with_specific_path(client_installation.temp_dir() / 'temp20880_lowres.mp4')
        local_file_node = ResourceTree(testkit_api, hid).wait_for_local_file('temp20880_lowres.mp4')
        local_file_node.open_in_new_tab().wait_for_accessible()

        TimelineNavigation(testkit_api, hid).pause_and_to_begin()
        loaded_lowres_playback = SavedImage(gui_prerequisite_store.fetch('test20880/screen_lowres.png'))
        scene = Scene(testkit_api, hid)
        scene.wait_until_first_item_is_similar_to(loaded_lowres_playback)
        assert scene.get_first_item().has_timestamp_on_position(timestamp_position)


if __name__ == '__main__':
    exit(test_export_low_resolution_video_with_overlays().main())
