...
class test_login_success(WebAdminTest):
    """Test login success.

    Selection-Tag: web-admin-gitlab
    TestRail: https://networkoptix.testrail.net/index.php?/cases/view/79734
...
def _test(args, exit_stack: ExitStack):
    ...
    assert _user_is_logged_in(browser)


def _user_is_logged_in(browser: Browser) -> bool:
    return element_is_present(UpperMenu(browser).get_account_settings)


if __name__ == '__main__':
    exit(run_ft_test(sys.argv, [test_login_success()]))
