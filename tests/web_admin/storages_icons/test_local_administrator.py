...
class test_local_administrator(WebAdminTest):
    """Test storages icons.
...
def _test(args, exit_stack: ExitStack):
    ...
    group_id = mediaserver_api.add_user_group('local_group', [Permissions.ADMIN])
    mediaserver_api.add_local_user(username, password, group_id=group_id)
    ...
    login_form.get_login_field().put(username)
    login_form.get_password_field().put(password)
...
if __name__ == '__main__':
    exit(run_ft_test(sys.argv, [test_local_administrator()]))
