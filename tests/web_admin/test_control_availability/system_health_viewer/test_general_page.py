...
class test_pages_control_availability(WebAdminTest):
    ...
    TestRail: https://networkoptix.testrail.net/index.php?/cases/view/122530
    ...
    def _run(self, args, exit_stack: ExitStack):
        ...
        if distrib.newer_than('vms_6.0'):
            raise BranchNotSupported(
                "Skipped due to https://networkoptix.atlassian.net/browse/CLOUD-13634")
        ...
        health_viewer = "health_viewer"
        health_viewer_password = "health_viewer_password"
        mediaserver_api.add_local_user(
            health_viewer, health_viewer_password, group_id=Groups.HEALTH_VIEWERS)
        ...
        login_form.get_login_field().put(health_viewer)
        login_form.get_password_field().put(health_viewer_password)
        ...
        expected_permissions = 'System Health Viewers'
        permissions = system_form.get_permissions().get_text()
        assert permissions == expected_permissions, f"{permissions} != {expected_permissions}"
