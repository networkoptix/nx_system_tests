...
def _test(args, exit_stack: ExitStack):
    ...
    assert all_access_rights_group_name in permission_groups_text, (
        f"{all_access_rights_group_name!r} not in {permission_groups_text!r}"
        )
    assert no_access_rights_group_name in permission_groups_text, (
        f"{no_access_rights_group_name!r} not in {permission_groups_text!r}"
        )
