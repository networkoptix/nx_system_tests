...
class test_control_availability(WebAdminTest):
    ...
    def _run(self, args, exit_stack: ExitStack):
        ...
        camera_credentials.get_login_input().put("IRRELEVANT")
        ...
        assert login_after == "IRRELEVANT", f"Login is not 'IRRELEVANT': {login_after}"
...
def _change_to_next_rotation_angle(camera: Camera, browser: Browser):
    ...
    for name, available_rotation_angle in available_angles.items():
        ...
        # For unknown reason, rotation angle after get changed in the web interface is not change
        # yet for a while. Saving changes just after change a rotation angle won't save it.
        # No anchors were found. The feed picture rotation is not enough.
        time.sleep(1)
        return
