...
from tests.web_admin._network_interfaces_info_page import NetworkInterfacesInfoTable
from tests.web_admin._network_interfaces_info_page import get_interfaces_search_input
from tests.web_admin._upper_menu import UpperMenu
from vm.networks import setup_flat_network


class test_network_interfaces_info(WebAdminTest):
...
def _test(args, exit_stack: ExitStack):
    ...
    InformationMenu(browser).get_network_interfaces_link().invoke()
    info_table = NetworkInterfacesInfoTable(browser)
    ...
    order = info_table.state_order()
    order.set_ascending()
    assert browser.get_current_url().endswith("sortBy=info,state,ASC")
    order.set_descending()
    assert browser.get_current_url().endswith("sortBy=info,state,DESC")

    order = info_table.ip_order()
    order.set_ascending()
    assert browser.get_current_url().endswith("sortBy=info,displayAddress,ASC")
    order.set_descending()
    assert browser.get_current_url().endswith("sortBy=info,displayAddress,DESC")

    order = info_table.in_rate_order()
    order.set_ascending()
    assert browser.get_current_url().endswith("sortBy=rates,inBps1m,ASC")
    order.set_descending()
    assert browser.get_current_url().endswith("sortBy=rates,inBps1m,DESC")

    order = info_table.out_rate_order()
    order.set_ascending()
    assert browser.get_current_url().endswith("sortBy=rates,outBps1m,ASC")
    order.set_descending()
    assert browser.get_current_url().endswith("sortBy=rates,outBps1m,DESC")

    get_interfaces_search_input(browser).put("Nonexistent_interface")
...
if __name__ == '__main__':
    exit(run_ft_test(sys.argv, [test_network_interfaces_info()]))
