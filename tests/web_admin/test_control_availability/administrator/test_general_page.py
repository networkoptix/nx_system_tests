...
def _test(args, exit_stack: ExitStack):
    ...
    assert system_form.get_merge_with_another_button().is_active()
    system_name_field = system_form.get_editable_name()
    assert system_name_field.get_current_value() == system_name
    assert not element_is_present(system_form.get_permissions), (
        "The permission field must be absent for the built-in Administrator user"
        )
