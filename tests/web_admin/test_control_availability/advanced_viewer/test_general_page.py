...
class test_ubuntu22(WebAdminTest):
    ...
    TestRail: https://networkoptix.testrail.net/index.php?/cases/view/122527
    """
...
def _test_pages_control_availability(args, one_vm_type: str, exit_stack: ExitStack):
    ...
    mediaserver_api.add_local_user(
        advanced_viewer, advanced_viewer_password, group_id=Groups.ADVANCED_VIEWERS)
    ...
    login_form.get_login_field().put(advanced_viewer)
    login_form.get_password_field().put(advanced_viewer_password)
    ...
    assert not nx_cloud_form.get_cloud_connect_link().get_full_url()
    system_name_field = system_form.get_editable_name()
    assert system_name_field.get_current_value() == system_name
    assert system_form.get_permissions().get_text() == GroupNames.ADVANCED_VIEWERS
