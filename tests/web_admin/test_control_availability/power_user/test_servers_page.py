...
def _test_pages_control_availability(args, one_vm_type: str, exit_stack: ExitStack):
    ...
    assert not _detach_from_system_button_is_present(first_server_settings)
    assert not _reset_to_default_button_is_present(first_server_settings)
    assert not first_server_settings.get_port().is_active()
    ...
    assert not _detach_from_system_button_is_present(second_server_settings)
    assert not _reset_to_default_button_is_present(second_server_settings)
    assert not second_server_settings.get_port().is_active()
...
def _detach_from_system_button_is_present(server_settings: ServerSettings) -> bool:
    try:
...
def _reset_to_default_button_is_present(server_settings: ServerSettings) -> bool:
    try:
