...
def _test(args, exit_stack: ExitStack):
    ...
    assert camera_component.get_detailed_info_button().is_active()
...
def _change_to_next_aspect_ratio(camera: Camera, browser: Browser):
    ...
    for name, available_aspect_ratio in available_aspect_ratios.items():
        ...
        # For unknown reason, aspect ratio in the web interface is not really changed
        # just after the click. Saving changes just after change an aspect ratio won't save it.
        # No anchors were found. Is not reproduced at manual testing.
        time.sleep(1)
        return
