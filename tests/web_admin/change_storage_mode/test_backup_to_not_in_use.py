...
class test_backup_to_not_in_use(WebAdminTest):
    """Change storage mode from Backup to Not in use.

    See: https://networkoptix.atlassian.net/browse/CLOUD-14758
    TestRail: https://networkoptix.testrail.net/index.php?/cases/view/84331
    """
...
def _test(args, exit_stack: ExitStack):
    ...
    _discovered, secondary_saved = mediaserver_api.set_up_new_storage(
        secondary_storage_path, is_backup=True)
    ...
    _wait_non_empty(camera_primary_archive, timeout=mediafile_duration * 5)
    ...
    secondary_storage_entry = storages_table.find_storage_entry(secondary_storage_path)
    secondary_storage_entry.get_mode().invoke()
    ...
    try:
        ...
        if 'Recording to this storage location will stop' in str(err):
            raise RuntimeError((
                "Stop recording warning badge is not found. "
                "See: https://networkoptix.atlassian.net/browse/CLOUD-14758"
                ))
        raise
    ...
    _wait_active(camera_primary_archive, timeout=mediafile_duration * 3)
    secondary_periods = camera_secondary_archive.list_periods()
    assert not secondary_periods, f"Secondary periods are not empty: {secondary_periods}"
...
if __name__ == '__main__':
    exit(run_ft_test(sys.argv, [test_backup_to_not_in_use()]))
