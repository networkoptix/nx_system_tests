...
class test_from_backup_to_main(WebAdminTest):
    """Change storage mode from Backup to Main.

    Selection-Tag: web-admin-gitlab
    TestRail: https://networkoptix.testrail.net/index.php?/cases/view/84327
    """
...
def _test(args, exit_stack: ExitStack):
    ...
    backup_storage_path = str(_add_usb_storage(mediaserver_vm, 'T', 300 * 1024**3))
    ...
    [_discovered, saved] = mediaserver_api.set_up_new_storage(backup_storage_path, is_backup=True)
    ...
    backup_archive = mediaserver.archive(saved.path)
    camera_backup_archive = backup_archive.camera_archive(single_camera.physical_id)
    with mediaserver_api.camera_recording(single_camera.id):
        camera_server.serve(time_limit_sec=5)
    assert len(camera_main_archive.high().list_periods()) == 1
    assert len(camera_backup_archive.high().list_periods()) == 0
    ...
    backup_storage_entry.get_mode().invoke()
    choice_menu = storage_mode_choice_menu(browser, installer_supplier.distrib().version())
    choice_menu.get_main_entry().choose()
    NxApplyBar(browser).get_cancel_button().invoke()
    with mediaserver_api.camera_recording(single_camera.id):
        camera_server.serve(time_limit_sec=5)
    assert len(camera_main_archive.high().list_periods()) == 2
    assert len(camera_backup_archive.high().list_periods()) == 0
    storages_table = storage_locations.get_storages_table()
    backup_storage_entry = storages_table.find_storage_entry(backup_storage_path)
    backup_storage_entry.get_mode().invoke()
    ...
    _wait_storage_become_main(storage_locations, backup_storage_path, timeout=10)
    with mediaserver_api.camera_recording(single_camera.id):
        camera_server.serve(time_limit_sec=5)
    assert len(camera_main_archive.high().list_periods()) == 2
    assert len(camera_backup_archive.high().list_periods()) == 1
...
def _wait_storage_become_main(
    ...
    while True:
        try:
            ...
            if storage_entry.get_mode().get_text() == 'Main':
                return
        ...
        if end_at < time.monotonic():
            raise RuntimeError("Storage mode has not changed from Main to Backup")
        time.sleep(0.5)
...
if __name__ == '__main__':
    exit(run_ft_test(sys.argv, [test_from_backup_to_main()]))
