...
class test_from_disabled_to_main(WebAdminTest):
    """Change storage mode from Disabled to Main.

    Selection-Tag: web-admin-gitlab
    TestRail: https://networkoptix.testrail.net/index.php?/cases/view/84329
    """
...
def _test(args, exit_stack: ExitStack):
    ...
    _wait_storage_become_main(storage_locations, disabled_storage_path, timeout=10)
    with mediaserver_api.camera_recording(single_camera.id):
        camera_server.serve(time_limit_sec=10)
    main_periods = camera_main_archive.high().list_periods()
    disabled_periods = camera_disabled_archive.high().list_periods()
    assert len(main_periods) == 1, f"Main: {main_periods}, Disabled: {disabled_periods}"
    assert len(disabled_periods) == 1, f"Main: {main_periods}, Disabled: {disabled_periods}"
...
if __name__ == '__main__':
    exit(run_ft_test(sys.argv, [test_from_disabled_to_main()]))
