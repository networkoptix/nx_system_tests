...
class test_advanced_viewer(WebAdminTest):
...
def _test(args, exit_stack: ExitStack):
    ...
    # 2) User2 (Advanced Viewers)
    # is tested here to ensure better granularity.
    ...
    local_user_name = "local_advanced_viewer"
    local_user_password = "local_advanced_viewer_password"
    mediaserver_api.add_local_user(
        local_user_name,
        local_user_password,
        group_id=Groups.ADVANCED_VIEWERS,
        )
    ...
    user_entry.open()
    expected_group_name = GroupNames.POWER_USERS
    _change_user_group(browser, from_=GroupNames.ADVANCED_VIEWERS, to=expected_group_name)
    group_name_in_form = UserForm(browser).get_change_group_button().get_text()
...
if __name__ == '__main__':
    exit(run_ft_test(sys.argv, [test_advanced_viewer()]))
