...
def _test_nas_reserved_space(distrib_url, two_vm_types, api_version, exit_stack):
    ...
    small = add_smb_storage(api, smb_machine.os_access, str(smb_ip), 300 * 1024**3)
    medium = add_smb_storage(api, smb_machine.os_access, str(smb_ip), 512 * 1024**3)
    large = add_smb_storage(api, smb_machine.os_access, str(smb_ip), 1100 * 1024**3)
