...
class test_v4plus(VMSTest):
    ...
    def _run(self, args, exit_stack):
        _test_update_on_scheme_square(args.distrib_url, 'v4plus', exit_stack)


def _test_update_on_scheme_square(distrib_url, api_version, exit_stack):
    ...
    [system, _, _] = exit_stack.enter_context(pool.system({
        ...
        'networks': {
            ...
            '10.254.3.0/24': {'proxy-2': None, 'second': None},
            '10.254.4.0/24': {'first': None, 'proxy-2': None},
            },
        }))
    setup_system(system, [
        ...
        {'local': 'proxy-2', 'network': '10.254.3.0/24', 'remote': 'second', 'take_remote_settings': True},
        ])
