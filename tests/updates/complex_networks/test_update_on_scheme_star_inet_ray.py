...
class test_v4plus(VMSTest):
    """Test update on scheme star inet ray.
    ...
    def _run(self, args, exit_stack):
        _test_update_on_scheme_star_inet_ray(args.distrib_url, 'v4plus', exit_stack)


def _test_update_on_scheme_star_inet_ray(distrib_url, api_version, exit_stack):
    ...
    [system, _, _] = exit_stack.enter_context(pool.system({
        ...
        'mergers': [
            ...
            {'local': 'third', 'network': '10.254.3.0/24', 'remote': 'hub', 'take_remote_settings': True},
            ],
        'networks': {
            '10.254.1.0/24': {'first': None, 'hub': None},
            '10.254.2.0/24': {'hub': None, 'second': None},
            '10.254.3.0/24': {'hub': None, 'third': None},
            },
        }))
