...
class test_v4plus(VMSTest):
    """Test update on scheme triangle additional.
    ...
    def _run(self, args, exit_stack):
        _test_update_on_scheme_triangle_additional(args.distrib_url, 'v4plus', exit_stack)


def _test_update_on_scheme_triangle_additional(distrib_url, api_version, exit_stack):
    ...
    [system, _, _] = exit_stack.enter_context(pool.system({
        ...
        'mergers': [
            {'local': 'first', 'network': '10.254.1.0/24', 'remote': 'fourth', 'take_remote_settings': True},
            {'local': 'second', 'network': '10.254.2.0/24', 'remote': 'first', 'take_remote_settings': True},
            {'local': 'third', 'network': '10.254.3.0/24', 'remote': 'second', 'take_remote_settings': True},
            ],
        'networks': {
            '10.254.1.0/24': {'first': None, 'fourth': None},
            '10.254.2.0/24': {'first': None, 'second': None},
            '10.254.3.0/24': {'second': None, 'third': None},
            '10.254.4.0/24': {'first': None, 'third': None},
            },
        }))
