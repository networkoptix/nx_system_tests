...
class test_v4plus(VMSTest):
    """Test update on scheme layout 1 2 hub 3 4.
    ...
    def _run(self, args, exit_stack):
        _test_update_on_scheme_layout_1_2_hub_3_4(args.distrib_url, 'v4plus', exit_stack)


def _test_update_on_scheme_layout_1_2_hub_3_4(distrib_url, api_version, exit_stack):
    ...
    [system, _, _] = exit_stack.enter_context(pool.system({
        'machines': [
            ...
            {'alias': 'hub', 'type': 'ubuntu22'},
            ],
        'mergers': [],
        'networks': {
            '10.254.1.0/24': {'first': None, 'hub': None},
            '10.254.2.0/24': {'hub': None, 'second': None},
            '10.254.3.0/24': {'first': None, 'third': None},
            '10.254.4.0/24': {'fourth': None, 'second': None},
            },
        }))
    setup_system(system, [
        {'local': 'first', 'network': '10.254.1.0/24', 'remote': 'hub', 'take_remote_settings': True},
        {'local': 'second', 'network': '10.254.2.0/24', 'remote': 'hub', 'take_remote_settings': True},
        {'local': 'third', 'network': '10.254.3.0/24', 'remote': 'first', 'take_remote_settings': True},
        {'local': 'fourth', 'network': '10.254.4.0/24', 'remote': 'second', 'take_remote_settings': True},
        ])
