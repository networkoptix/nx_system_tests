...
class test_v0(VMSTest):
    ...
    def _run(self, args, exit_stack):
        _test_merge_by_scheme_triangle_pull(args.distrib_url, 'v0', exit_stack)


def _test_merge_by_scheme_triangle_pull(distrib_url, api_version, exit_stack):
    ...
    [mediaservers, _, _] = exit_stack.enter_context(pool.system({
        ...
        'networks': {
            ...
            '10.254.3.0/24': {'second': None, 'first': None},
            },
        'mergers': [],
        }))
    setup_system(mediaservers, [
        {'local': 'proxy-1', 'remote': 'first', 'take_remote_settings': True, 'network': '10.254.1.0/24'},
        {'local': 'second', 'remote': 'first', 'take_remote_settings': False, 'network': '10.254.3.0/24'},
        ])
