...
def _test_can_see_cameras_created_by_admin(distrib_url, one_vm_type, api_version, actor, exit_stack):
    ...
    response = mediaserver_api_for_actor.list_cameras()
    camera_ids = [c.id for c in response]
    assert dummy_camera.id in camera_ids
