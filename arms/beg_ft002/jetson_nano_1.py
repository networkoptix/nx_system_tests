...
from arms.beg_ft002.local_resources import get_jetson_nano_named_distributor
from arms.beg_ft002.local_resources import jetson_nano_installation_distributor
from arms.beg_ft002.local_resources import jetson_nano_prerequisites_distributor
...
_router = '10.1.0.220'
_name = 'jetson-nano-1'
_mac = '48:b0:2d:5b:5b:19'


_contractor = SnapshotContractor(
    machine=PXEMachine(
        name=_name,
        power_interface=MikrotikPowerControl(poe_switch, 2),
        remote_control=SSHRemoteControl(host=_ip, port=22, user="root", ssh_key=ssh_key),
        tftp_boot_loader=JetsonNanoTFTPBootloader(local_ip=_ip, mac=_mac),
...
async def main():
    async with aclosing(status_endpoint(_name)) as machine_status:
        await serve_contracts_loop(
            ...
            distributors=[
                get_jetson_nano_named_distributor(_name),
                jetson_nano_prerequisites_distributor,
                jetson_nano_installation_distributor,
                ],
